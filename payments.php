...
$total_paid=0;
$r=$db->run("SELECT SUM(val) as v, address FROM payments WHERE height<:h AND height>=:h2 AND done=0 GROUP by address",array(":h"=>$current-10, ":h2"=>$current-$blocks_paid));
foreach($r as $x){
	if($x['v']<$pool_config['min_payout']) continue;
		$fee=$x['v']*0.0025;
